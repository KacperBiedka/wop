   ...
   const updateSetsState = (e) => {
    ...
    const AddExercise = () => {
        ...
        firebase.auth().onAuthStateChanged(function(user) {
          if (user) {
            const uid = user.uid;
            const query = db.collection("workouts").where("uid", "==", uid).where("workoutNumber", "==", this.state.workoutNumber).limit(1)
            query.get()
            .then((snapshot) => {
              snapshot.docs.forEach(doc => {
                ...
                .then(() => {
                  this.props.getExercisesToRedux(exercisesCopy, this.state.workoutNumber);
    ...
    return (
        <>
          <Modal show={this.props.visible} onHide={this.props.closeModal}>
            ...
            <Modal.Body>
              <h5 className={classes.exerciseHeader}>Exercise {(this.state.exercisesState != null) ? this.state.exercisesState.length + 1 : null}</h5>
